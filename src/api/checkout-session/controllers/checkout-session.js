...
const populatePurchases = async () => {
  ...
  const checkouts = await strapi.service("api::checkout-session.checkout-session").find();
    console.log(checkouts, "WHYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY")
  for (const checkout of checkouts.results) {
    const { customer_name, paymongo_checkout_id, products, createdAt } = checkout;
